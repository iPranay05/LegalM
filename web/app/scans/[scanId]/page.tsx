...
export default function ScanDetailPage() {
  ...
  if (pipelineStatus === "pending" || pipelineStatus === "processing") {
    return (
      <div className="max-w-2xl mx-auto my-12 lm-card p-8 text-center space-y-4">
        ...
        <h2 className="text-xl font-bold text-on-surface">Processing Perception Pipeline</h2>
  ...
  if (!scan || !tabs) {
    return (
      <div className="text-center py-12 text-on-surface-variant">
        Scan not found. <Link href="/scans" className="text-secondary underline">Back to scans</Link>
      </div>
    );
  }
  ...
  const tabConfig: { id: Tab; label: string; count: number; color: string }[] = [
    { id: "all", label: "All Rules", count: tabs.all_rules.length, color: "text-on-surface-variant" },
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-2 text-sm text-on-surface-variant">
          ...
          <span>/</span>
          <Link href="/scans" className="hover:text-secondary">Scan History</Link>
          <span>/</span>
          <span className="text-on-surface font-medium truncate max-w-xs">{scanId.slice(0, 8)}</span>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link href="/scans/new" className="lm-btn-primary !py-1.5 !px-3 text-xs">
            <span className="material-symbols-outlined text-[16px]">add_a_photo</span>
            Scan Another Product
          </Link>
          <Link href="/scans" className="lm-btn-secondary !py-1.5 !px-3 text-xs">
      ...
      {/* Top card — Compliance Result summary */}
      <div className="lm-card p-6">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            <p className="font-label-caps text-label-caps text-on-surface-variant m-0 mb-1">Compliance Result</p>
            <h1 className="font-headline-lg text-headline-lg text-on-surface m-0">{scan.product_name || "Unknown Product"}</h1>
            <p className="text-sm text-on-surface-variant mt-0.5 font-data-mono text-data-mono">{scan.scan_id}</p>
          ...
          <div className="flex flex-col items-start md:items-end gap-2">
            ...
            <p className="text-xs text-on-surface-variant">Scanned {formatDate(scan.created_at)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="lm-card p-5">
            <h2 className="text-xs font-bold text-on-surface-variant uppercase tracking-wide mb-4">Scan Details</h2>
            <dl className="grid grid-cols-2 md:grid-cols-3 gap-x-6 gap-y-3">
              {[
                ...
                <div key={label}>
                  <dt className="text-xs text-on-surface-variant font-semibold uppercase">{label}</dt>
                  <dd className="text-sm text-on-surface font-medium mt-0.5">{value}</dd>
                </div>
              ))}
            </dl>
            ...
            {scan.symbols_detected && (
              <div className="mt-4 pt-4 border-t border-border-subtle">
                <p className="text-xs font-bold text-on-surface-variant uppercase tracking-wide mb-2">Symbol Detection</p>
                <div className="flex gap-3 flex-wrap">
                  <span className={cn("text-xs font-bold px-2.5 py-1 rounded-full border",
                    scan.symbols_detected.veg_dot ? "bg-green-50 text-green-700 border-green-200" : "bg-surface-container-low text-on-surface-variant border-border-subtle")}>
                    🟢 Veg Dot: {scan.symbols_detected.veg_dot ? "Detected" : "Not Found"}
                  </span>
                  <span className={cn("text-xs font-bold px-2.5 py-1 rounded-full border",
                    scan.symbols_detected.non_veg_dot ? "bg-red-50 text-red-700 border-red-200" : "bg-surface-container-low text-on-surface-variant border-border-subtle")}>
                    🔴 Non-Veg Dot: {scan.symbols_detected.non_veg_dot ? "Detected" : "Not Found"}
                  </span>
                  <span className={cn("text-xs font-bold px-2.5 py-1 rounded-full border",
                    scan.symbols_detected.gm_mark ? "bg-orange-50 text-orange-700 border-orange-200" : "bg-surface-container-low text-on-surface-variant border-border-subtle")}>
          ...
          <div className="lm-card overflow-hidden">
                ...
                >
                  ...
                  {t.count > 0 && (
                    <span className={cn("ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full",
                      activeTab === t.id ? "bg-secondary text-white" : "bg-surface-container text-on-surface-variant"
                    )}>
                      {t.count}
                    </span>
                  )}
                  ...
                  {tabs.all_rules.map((row) => (
                    <tr key={row.key} className="hover:bg-surface-container-low">
                      <td className="table-td">
                        ...
                        {row.legal_reference && <p className="text-xs text-on-surface-variant mt-0.5">{row.legal_reference}</p>}
                      </td>
                      <td className="table-td">
                        {row.required
                      ...
                      <td className="table-td text-xs font-mono text-on-surface-variant">{row.extracted_value || "—"}</td>
                      <td className="table-td">
                    ...
                    <tbody>
                      {tabs.violations.map((row) => (
                        <tr key={row.key} className="hover:bg-red-50/40">
                          ...
                          <td className="table-td text-xs text-on-surface-variant">{row.legal_reference || "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            {activeTab === "relaxed" && (
              tabs.not_applicable_relaxed.length === 0 ? (
                <div className="text-center py-12 text-on-surface-variant text-sm">No optional fields absent.</div>
              ) : (
                <table className="w-full">
                  <thead><tr>
            ...
            {activeTab === "findings" && (
              <div>
                ...
                {/* Add finding form */}
                {findingForm.show && (
                  <form onSubmit={submitFinding} className="px-5 py-4 bg-surface-container-low border-b border-border-subtle space-y-3">
                    ...
                    <div className="flex justify-end gap-2">
                      <button type="button" onClick={() => setFindingForm(f => ({ ...f, show: false }))} className="lm-btn-secondary !text-xs !py-1.5 !px-3">Cancel</button>
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-2">
                            <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase",
                              SEVERITY_COLORS[f.severity] || "bg-surface-container text-on-surface-variant border-border-subtle")}>
                              {f.severity}
                            </span>
                            <span className="text-xs font-bold text-on-surface uppercase">{f.finding_type}</span>
                            {f.rule_code && <span className="text-xs font-mono text-on-surface-variant">{f.rule_code}</span>}
                          </div>
                          <p className="text-sm text-on-surface">{f.description}</p>
                          {f.evidence_note && <p className="text-xs text-on-surface-variant italic">{f.evidence_note}</p>}
                        </div>
                        <button onClick={() => deleteFinding(f.id)} className="text-on-surface-variant hover:text-red-600 text-sm">✕</button>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div className="lm-card p-5">
            ...
            >
              <div>
                <span className="text-xs font-bold text-on-surface-variant uppercase tracking-wide">Raw OCR Text</span>
                <p className="text-xs text-on-surface-variant mt-0.5">Extracted using Tesseract OCR + Groq Vision</p>
              </div>
              <span className="text-xs text-secondary font-semibold">{showOcr ? "Hide ▲" : "Show ▼"}</span>
            </button>
            {showOcr && (
              <pre className="mt-4 p-4 bg-surface-container-low rounded-lg text-xs font-mono text-on-surface whitespace-pre-wrap max-h-60 overflow-y-auto border border-border-subtle">
        ...
        <div className="space-y-5">
          ...
          <div className="lm-card p-5">
            <h2 className="text-xs font-bold text-on-surface-variant uppercase tracking-wide mb-3">Product Label Photo</h2>
            {imageUrl ? (
              <div className="relative rounded-lg overflow-hidden border border-border-subtle bg-surface-container-low">
                {imageCount > 1 && (
                  <>
              ...
              <div className="h-48 bg-surface-container-low rounded-lg flex items-center justify-center text-on-surface-variant text-sm">
                No image available
              </div>
            )}
            {imageCount > 1 && <p className="mt-2 text-center text-xs text-on-surface-variant">Photo {imageIndex + 1} of {imageCount} · swipe or use arrows</p>}

            {/* Bounding box review list */}
            {scan.bounding_boxes && scan.bounding_boxes.length > 0 && (
              <div className="mt-4 space-y-2">
                <p className="text-xs font-bold text-on-surface-variant uppercase">Detection Bounding Boxes</p>
                {scan.bounding_boxes.map((b) => (
                  ...
                  >
                    <div>
                      <span className="font-bold text-on-surface">{FIELD_LABELS[b.field] || b.field}</span>
                      {b.text && <p className="text-on-surface-variant font-mono text-[11px] mt-0.5">{b.text}</p>}
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      {!b.confirmed && (
                        <button
                          ...
                          className="text-[11px] bg-secondary text-white px-2 py-0.5 rounded hover:bg-secondary/80 disabled:opacity-50"
                        >
                          Confirm
                        </button>
                      )}
          ...
          {/* Barcode details */}
          <div className="lm-card p-5">
            <h2 className="text-xs font-bold text-on-surface-variant uppercase tracking-wide mb-3">Barcode & External Registry</h2>
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <span className="text-2xl">{scan.barcode_data?.decoded ? "📦" : "🔍"}</span>
                <div>
                  <p className="text-sm font-semibold text-on-surface">Barcode Detection</p>
              ...
              {scan.barcode_data?.decoded && scan.barcode_data.barcodes?.length > 0 && (
                <div className="space-y-2">
                  ...
                  {scan.barcode_data.barcodes.map((bc, i) => (
                    <div key={i} className="flex items-center justify-between bg-surface-container-low rounded-lg px-3 py-2 border border-border-subtle">
                      <div>
                        <span className="text-[10px] font-bold text-secondary bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded mr-2">{bc.type}</span>
                        <span className="text-xs font-mono text-on-surface">{bc.data}</span>
                      </div>
                      {bc.data === scan.barcode_data?.primary_barcode && (
                        <span className="text-[10px] text-on-surface-variant font-semibold">Primary</span>
                      )}
                    </div>
                  ))}
          ...
          {/* Score summary */}
          <div className="lm-card p-5">
            <h2 className="text-xs font-bold text-on-surface-variant uppercase tracking-wide mb-4">Compliance Score</h2>
            <div className="space-y-4">
              <div>
                <div className="flex justify-between text-xs text-on-surface-variant mb-1.5">
                  <span>Overall Score</span>
                  <span className="font-bold text-on-surface">{scan.compliance_score?.toFixed(1) ?? 0}%</span>
                </div>
              ...
              <div className="pt-2 border-t border-border-subtle space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-on-surface-variant">Total rules</span>
                ...
                <div className="flex justify-between">
                  <span className="text-on-surface-variant">Manual findings</span>
          ...
          {/* Report generation */}
          <div className="lm-card p-5">
            <h2 className="text-xs font-bold text-on-surface-variant uppercase tracking-wide mb-3">Generate Report</h2>
            ...
            {reports.length > 0 && (
              <div className="mt-4 space-y-2">
                <p className="text-xs font-bold text-on-surface-variant uppercase">Generated Reports</p>
                {reports.map((r) => (
                  <div key={r.report_id} className={cn(
                    "flex items-center justify-between rounded-lg px-3 py-2 text-xs border",
                    r.superseded_by_report_id ? "opacity-50 bg-surface-container-low border-border-subtle" : "bg-white border-border-subtle"
                  )}>
                    <div>
                      <span className="font-bold uppercase text-on-surface mr-2">{r.format}</span>
                      <span className="text-on-surface-variant">{formatDate(r.generated_at, "dd MMM, hh:mm a")}</span>
                      {r.superseded_by_report_id && <span className="ml-2 text-on-surface-variant italic">(superseded)</span>}
                    </div>
                    <button
                      onClick={() => downloadReport(r.report_id, r.format)}
                      className="text-secondary font-semibold hover:underline text-xs"
                    >
                      Download ↓
                    </button>
                  </div>

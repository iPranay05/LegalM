...
# ─── 5. Veg / Non-Veg Symbol (FSS Labelling & Display Regulations, 2020) ────────

def test_veg_non_veg_rule_is_seeded_and_active_for_food(db_session: Session):
    """The Symbol rule for veg/non-veg must exist and resolve for the Food category."""
    food_cat = db_session.query(CommodityCategory).filter(CommodityCategory.name == "Food").first()
    rule = db_session.query(Rule).filter(Rule.rule_family == "veg_non_veg_symbol").first()
    assert rule is not None
    assert rule.check_type == RuleCheckType.Symbol

    active = get_active_rules(category=food_cat, evaluation_date=date(2026, 1, 1), db=db_session)
    assert any(r.rule_family == "veg_non_veg_symbol" for r in active)


def test_veg_non_veg_symbol_detected_passes_food_scan(db_session: Session):
    """Food-category scan where OpenCV/vision detected the veg dot -> Pass, and it
    is included in compliance_checks (previously silently dropped from reports)."""
    food_cat = db_session.query(CommodityCategory).filter(CommodityCategory.name == "Food").first()
    context = {
        "ocr_text": "Sample food product label",
        "commodity_category": food_cat,
        "symbols": {"veg_dot": True, "non_veg_dot": False},
        "calibration": {"method": "unverified"},
        "db": db_session,
    }
    checks = evaluate_declarations(context)
    symbol_check = next(c for c in checks if c.field_key == "veg_non_veg_symbol")
    assert symbol_check.result == "Pass"


def test_veg_non_veg_symbol_missing_fails_food_scan(db_session: Session):
    """Food-category scan where neither mark was detected -> Fail."""
    food_cat = db_session.query(CommodityCategory).filter(CommodityCategory.name == "Food").first()
    context = {
        "ocr_text": "Sample food product label",
        "commodity_category": food_cat,
        "symbols": {"veg_dot": False, "non_veg_dot": False},
        "calibration": {"method": "unverified"},
        "db": db_session,
    }
    checks = evaluate_declarations(context)
    symbol_check = next(c for c in checks if c.field_key == "veg_non_veg_symbol")
    assert symbol_check.result == "Fail"
    assert "not detected" in symbol_check.notes.lower()


def test_veg_non_veg_symbol_not_applicable_for_general_category(db_session: Session):
    """General category is not food -> NotApplicable even if a mark happens to be visible,
    mirroring the existing FSSAI non-food exemption pattern."""
    general_cat = db_session.query(CommodityCategory).filter(CommodityCategory.name == "General").first()
    context = {
        "ocr_text": "Sample general product label",
        "commodity_category": general_cat,
        "symbols": {"veg_dot": False, "non_veg_dot": False},
        "calibration": {"method": "unverified"},
        "db": db_session,
    ...
    checks = evaluate_declarations(context)
    symbol_check = next(c for c in checks if c.field_key == "veg_non_veg_symbol")
    assert symbol_check.result == "NotApplicable"

...
# A few field keys read better with a custom label than a plain title-case
# split (e.g. "veg_non_veg_symbol" -> "Veg Non Veg Symbol" is not great).
_FIELD_LABEL_OVERRIDES = {
    "veg_non_veg_symbol": "Veg / Non-Veg Symbol",
    "fssai_number": "FSSAI Licence Number",
}


def _field_label(key: str) -> str:
    return _FIELD_LABEL_OVERRIDES.get(key, key.replace("_", " ").title())
    ...
    if checks:
        ...
        for check in checks:
            ...
            if bbox and bbox.get("bbox_source") != "vision_estimate":
                ...
                crop_bytes = _crop_bounding_box(img_path, bbox)
                if crop_bytes:
                    crops_to_embed.append((_field_label(key), res_val, conf_str, bbox.get("bbox_source"), crop_bytes))

...
def run_symbol_stage(image_bytes: bytes, ocr_text: str = "", category: str = "general",
                      vision_fields: Optional[dict] = None,
                      vision_confidences: Optional[dict] = None) -> dict:
    """
    Detect veg/non-veg dot and GM mark.

    OpenCV's color/shape heuristics look for a specific dot-inside-square
    geometry and can miss a genuine mark on a blurry, compressed, or
    glare-affected phone photo. When Groq vision was used for OCR, it has
    already looked at the same image and can directly report whether it saw
    the mark — used here as a second, independent signal and OR-combined
    with the OpenCV result so either method finding it is enough.
    """
    ...
    vision_confidences = vision_confidences or {}

    def _vision_bool(key: str) -> Optional[bool]:
        value = vision_fields.get(key)
        if isinstance(value, bool):
            return value
        if isinstance(value, str):
            low = value.strip().lower()
            if low in ("true", "yes", "present", "detected"):
                return True
            if low in ("false", "no", "absent", "not present", "not detected"):
                return False
        return None  # not visible in this image / not reported

    vision_veg = _vision_bool("veg_mark")
    if vision_veg is True and not symbols.get("veg_dot"):
        symbols["veg_dot"] = True
        symbols["veg_confidence"] = max(symbols.get("veg_confidence") or 0.0,
                                         vision_confidences.get("veg_mark") or 0.6)
        symbols["method"] = "opencv+vision"

    vision_nonveg = _vision_bool("non_veg_mark")
    if vision_nonveg is True and not symbols.get("non_veg_dot"):
        symbols["non_veg_dot"] = True
        symbols["non_veg_confidence"] = max(symbols.get("non_veg_confidence") or 0.0,
                                             vision_confidences.get("non_veg_mark") or 0.6)
        symbols["method"] = "opencv+vision"
...
def run_pipeline_sync(image_bytes: bytes,
    ...
    symbols = run_symbol_stage(image_bytes, raw_text, category,
                                vision_fields=extracted_fields,
                                vision_confidences=field_confidences)
...
def run_pipeline_task(self, scan_id: str, image_path: str | list[str], category: str):
    ...
    try:
        ...
        # Always clear any prior checks for this scan before (possibly) writing
        # new ones. Previously this delete only ran when checks_data was
        # non-empty, so a pipeline re-run that produced zero checks (e.g. rules
        # failed to load, category unmapped, or every field was confidence-gated
        # out) left the PREVIOUS run's rows in place. Reports then read
        # scan.compliance_checks and silently displayed stale/previous data.
